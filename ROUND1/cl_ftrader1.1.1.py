"""
IMC Prosperity 4 — Round 1 Trader (Strategy IMPROVED)
======================================================
ACO Improvements over Strategy A (offset=5):
  Fix 1: Inventory-skewed fair value (MOST IMPACTFUL)
  Fix 2: Penny-inside uses max() not assignment (prevents drag below target)
  Fix 3: Remove split quoting — all capacity at primary level

IPR: Unchanged from original.

=== DIAGNOSTICS FROM LOG (100k ts backtest) ===
Original ACO PnL:  3,229
Original trades:   91 fills  |  avg spread captured: 11.89 ticks

BUG 1 — Penny-inside drag:
  743/1000 ticks: penny logic pushed buy quote 3.3 ticks BELOW our own target
  Root cause: `buy_price = best_bid + 1` unconditionally replaces our target
  Fix: `buy_price = max(buy_price, best_bid + 1)`

BUG 2 — No inventory skew:
  261 ticks stuck short (<-20)  |  182 ticks stuck long (>+20) = 44.3% of session
  Root cause: FAIR is hardcoded 10000 regardless of inventory
  Fix: adj_fair = FAIR - 0.15 * pos
    → When short (-40): adj_fair=10006 → buy at 9999 vs old 9994 (+5 aggression)
    → When long (+40):  adj_fair=9994  → sell at 9999 vs old 10010 (-11 aggression)

BUG 3 — Wasted split order:
  Half capacity placed at buy_price-1 (often = existing market bid)
  No queue advantage, dilutes volume from primary level
  Fix: 100% at primary level

Expected PnL target: 4,000–5,000
"""
...
class Trader:
    ...
    # ── ACO parameters ──────────────────────────────────────────────────
    ACO_FAIR          = 10000
    ACO_OFFSET        = 5      # Base passive offset from adj_fair
    ACO_SKEW_ALPHA    = 0.15   # Ticks of fair adjustment per unit of position
    #                            pos=+40 → adj_fair=9994 → sell urgently at ~9999
    #                            pos=-40 → adj_fair=10006 → buy urgently at ~9999
    ACO_UNWIND_THRESH = 55     # Emergency unwind kicks in at this pos magnitude
    ACO_UNWIND_OFFSET = 2      # Emergency unwind posts at fair ± this

    # ── IPR parameters (unchanged) ──────────────────────────────────────
    IPR_BASE_POS = 75
    ...
    # ════════════════════════════════════════════════════════════════════
    # ACO — Improved market-making with inventory skew
    # ════════════════════════════════════════════════════════════════════
    def _trade_aco(self, state: TradingState, td: dict) -> tuple[list[Order], dict]:
        ...
        FAIR   = self.ACO_FAIR
        OFFSET = self.ACO_OFFSET

        buy_capacity  = limit - pos   # max units we can still buy
        sell_capacity = limit + pos   # max units we can still sell

        best_bid = max(od.buy_orders.keys())  if od.buy_orders  else None
        best_ask = min(od.sell_orders.keys()) if od.sell_orders else None

        # ── FIX 2: Inventory-skewed fair value ───────────────────────────
        # When long (pos > 0): adj_fair shifts DOWN → we quote lower → sell
        #   more aggressively & buy less aggressively → inventory unwinds.
        # When short (pos < 0): adj_fair shifts UP → we quote higher → buy
        #   more aggressively & sell less aggressively → covers short faster.
        adj_fair = FAIR - self.ACO_SKEW_ALPHA * pos

        # ── Take: buy every ask clearly below FAIR, sell every bid above FAIR
        # (Unchanged — these are genuine mispricings, not affected by skew)
        ...
        # ── Emergency unwind when position is extreme ─────────────────────
        # Fires only when abs(pos) > UNWIND_THRESH (55).
        # Posts at FAIR ± 2 for near-certain fills to prevent limit lock-out.
        if pos > self.ACO_UNWIND_THRESH and sell_capacity > 0:
            unwind_qty = min(pos - 40, sell_capacity)
            result.append(Order(product, FAIR + self.ACO_UNWIND_OFFSET, -unwind_qty))
            sell_capacity -= unwind_qty
        elif pos < -self.ACO_UNWIND_THRESH and buy_capacity > 0:
            unwind_qty = min(-pos - 40, buy_capacity)
            result.append(Order(product, FAIR - self.ACO_UNWIND_OFFSET, unwind_qty))
            buy_capacity -= unwind_qty

        # ── FIX 1 + FIX 2: Compute passive quote prices from adj_fair ─────
        #
        # BUY side:
        #   Target: floor(adj_fair) - OFFSET
        #   Penny-inside improvement: use max(target, best_bid+1)
        #     → OLD code used assignment   buy_price = best_bid+1
        #       which DRAGGED buy 3.3 ticks below target on 74% of ticks
        #     → NEW code uses max() so penny never goes BELOW our target
        #   Hard cap: FAIR - 1 (never buy at or above true fair)
        buy_price = math.floor(adj_fair) - OFFSET
        if best_bid is not None and (best_bid + 1) < adj_fair:
            buy_price = max(buy_price, best_bid + 1)   # ← FIX 1: max, not assign
        buy_price = min(buy_price, FAIR - 1)            # hard cap below true fair

        # SELL side (symmetric):
        #   Target: ceil(adj_fair) + OFFSET
        #   Penny-inside: use min(target, best_ask-1)
        #   Hard floor: FAIR + 1 (never sell at or below true fair)
        sell_price = math.ceil(adj_fair) + OFFSET
        if best_ask is not None and (best_ask - 1) > adj_fair:
            sell_price = min(sell_price, best_ask - 1)
        sell_price = max(sell_price, FAIR + 1)          # hard floor above true fair

        # ── FIX 3: Single-level passive quotes (no split) ─────────────────
        # OLD: split buy_capacity 50/50 between buy_price and buy_price-1.
        #   The buy_price-1 order often landed at the existing market bid
        #   level, adding no queue advantage and diluting our primary fill.
        # NEW: all capacity at the primary (best) price level.
        if buy_capacity > 0:
