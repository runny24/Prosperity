...
Round 5 v2
==========
Defensive correction after testing r5_v1.

The conservative replay showed that applying microstructure gating to all EMA
names hurt important baseline winners. v2 therefore keeps EMA execution close to
the original baseline, removes day-4 harness losers, and adds only the SNACKPACK
microstructure names that were positive standalone.

Changes versus the logged baseline:
- remove OXYGEN_SHAKE_MORNING_BREATH, ROBOT_MOPPING, and PANEL_1X4
- keep the PEBBLES_XL constraint arb
- keep strong EMA names such as GALAXY_SOUNDS_PLANETARY_RINGS, SLEEP_POD_COTTON,
  MICROCHIP_CIRCLE, MICROCHIP_OVAL, and TRANSLATOR_GRAPHITE_MIST
- add microstructure-only trades for SNACKPACK_PISTACHIO, SNACKPACK_STRAWBERRY,
