...
Round 5 v1
==========
Baseline v13 plus the first alpha improvements from commodity_alpha_exploration:

1. Keep the online-validated EMA roster and the PEBBLES_XL constraint arb.
2. Add a top-of-book microstructure overlay:
   - level-1 imbalance predicts next 1-5 ticks in the same direction
   - microprice edge predicts next 1-5 ticks in the same direction
   - level-3 imbalance predicts with the opposite sign
3. Add small SNACKPACK microstructure-only trades. The notebook showed unusually
   clean imbalance ICs in SNACKPACK, while the baseline did not trade them.

This version is intentionally conservative: it changes execution/target gating
before introducing the heavier family-relative mean-reversion module.
"""
...
class Trader:
    ...
    # These names were weak in the baseline day-4 log, so pure EMA entries now
    # require microstructure confirmation instead of just avoiding disagreement.
    EMA_NEEDS_MICRO_CONFIRM = {
    ...
    def _trade_ema(self, prod: str, od: OrderDepth, pos: int,
        ...
        target = self._micro_adjusted_target(prod, od, pos, target)
        allow_take = self._micro_allows_take(od, target - pos)
