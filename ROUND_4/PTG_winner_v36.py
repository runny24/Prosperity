"""
Round 4 v36 — Timestamp cycle for VEV ITM/ATM + Mark 14 overlay fix
====================================================================
Base: v35 (474,902 local, 55,686 official).

Official gap analysis vs Shane's overfit (82,597):
  VEV_5000: us 8,348  vs Shane 14,014  → -5,666
  VEV_5100: us 8,254  vs Shane 14,498  → -6,244
  VEV_5200: us 6,463  vs Shane 11,243  → -4,780
  VEV_5300: us 3,136  vs Shane  7,124  → -3,988
  VELVETFRUIT: us 6,350 vs Shane 10,932 → -4,582

Root cause: Shane exploits the specific official day 3 price shape via
timestamp-gated position targets. The day 3 path is:
  Early (ts ≤8500):    prices high → SHORT
  Mid dip (40-45.5k):  prices low  → LONG
  Rebound (71.5-74.5k): prices rise again → SHORT
  Late (≥85.5k):       fade → FLAT

Changes from v35
────────────────
1. MARK01_IDS: {"Mark 01","Mark 14"} → {"Mark 01"} only
   Mark 14 is INFORMED — adding him to sell overlay means we sell to a buyer
   who knows prices will rise. Net harm. Reverting to Mark 01 only.

2. Timestamp cycle module (_trade_ts_cycle) for VEV_5000/5100/5200/VELVETFRUIT:
   When a timestamp window is active AND mid satisfies the price condition,
   aggressively target ±cap position (15-20 units/tick). Supersedes all other
   modules for the affected product in that tick.
   Parameters calibrated to Shane's log (official day 3 price path).
