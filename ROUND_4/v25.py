"""
Round 4 v25 — SELL_MIN=7 + Mark 01 counterparty overlay
=========================================================
Base: R3_final (v23), verified locally on R4 30k data → 483,473 total PnL.

Changes from R3_final
─────────────────────
1. VEV_5500_SELL_MIN  8 → 7
   Sweep result (SLURM array, R4 30k data):
     SELL_MIN=5: -1,050  SELL_MIN=6: +1,355
     SELL_MIN=7: +2,560  SELL_MIN=8: +1,269  ← was baseline
   SELL_MIN=7 wins both VEV_5500 PnL and total PnL (+1,291 overall).

2. VOUCHER_TIME_VALUE unchanged from R3_final
   R4 order book mids confirm R3 TV values are accurate:
     VEV_5000 order book TV ≈ 6.3 (TV=5.0 → conservative buys, correct sells)
     VEV_5300 order book mid = 46.9 (TV=50 → threshold 47/52, barely fires)
     VEV_5400 order book mid = 15.7 (TV=16 → nearly exact calibration)
   Lowering TV (e.g., VEV_5300: 50→41) drops sell threshold from bid>52 to bid>43,
   causing constant selling into bid=46 order book at below-fair prices (−30k PnL
   verified in backtester). TV recalibration is NOT needed for R4 order book.

3. Mark 01 counterparty overlay (NEW)
   R4 introduces named counterparties in state.market_trades.
   Observed behaviour:
     Mark 01  — passive limit buyer of OTM VEVs (5300–6500); never sells.
     Mark 22  — aggressive option writer; hits Mark 01's bids.
   Signal: if Mark 01 appears as buyer in market_trades for a product,
   his limit orders were filled last tick and are likely still live.
   Response: sell additional size at best_bid if it clears a conservative floor.
   The overlay is additive to existing TV-take and extreme modules.
   MARK01_MAX_SELL caps exposure per tick per product.
