"""
Round 4 v42c — Safe frontier + moderate-risk bets (+9K additional from v42b) (+82K on top of v40)
========================================================================
Base: v39 (v31 + VEV_5500 cycling)

KEY INSIGHT FROM IV SMILE:
  Deep ITM options (VEV_4000/4500) have implied vol 0.45-0.68 vs smile-fair
  0.27 — 2x overpriced. This means every sell captures excess premium.
  The position cap of 200 was artificially limiting how much premium we harvest.

  Testing EXTREME_CAPS 200→250 on the 30K backtest:
    VEV_4000: +14,811 (D1:+4955, D2:+4206, D3:+5650) — ALL DAYS POSITIVE
    VEV_4500: +17,375 (D1:+5609, D2:+5914, D3:+5852) — ALL DAYS POSITIVE
    VEV_5000: +18,537 (D1:+10479, D2:+4728, D3:+3330) — ALL DAYS POSITIVE
    VEV_5100: +15,287 (D1:+9659, D2:+4913, D3:+715)  — ALL DAYS POSITIVE
    VEV_5200: +13,526 (D1:+9040, D2:+3966, D3:+520)  — ALL DAYS POSITIVE
    VEV_5300: already at 250 (no change)
    VEV_5400: already at 250 (no change)
    TOTAL: +85,670 with 100% day-level consistency across 7 products × 3 days

  This is the first parameter change that is positive on EVERY product on
  EVERY day. The hard position limit is 300; cap=250 still leaves 50 units
  of headroom for other modules.

Combined with v39's VEV_5500 fix (+7,650):
  Projected improvement: +93,320 → ~1,308,000 total

Round 4 v39 — VEV_5500 cycling re-enabled (+7,650 on 30K backtest)
=======================================================================
Base: v31 (1,214,631 on R4 30K backtest).

SINGLE CHANGE: VEV_5500 BUY_MAX 0→4, SELL_MIN 7→6.

Evidence from v38 backtest analysis:
  v38 changed 6 parameters from v31. Of those:
    VEV_5300 EXTREME_SELL 53→46:  -98,820  (TV-take/extreme conflict, all days)
    VELVETFRUIT EXTREME_BUY 5256: -18,085  (fires at fair, not extremes)
    VEV_5500 BUY_MAX=4/SELL_MIN=6: +7,650  (consistent +1,200/+3,600/+2,850 per day)
    VEV_5200: +8,935 (day 3 only, source unclear — interaction effect)
    VEV_5100: +4,361 (day 3 only, same)
    VEV_5000: +2,456 (likely from EXTREME_SELL 267, already in v31)

  VEV_5500 is the ONLY change that is:
    a) Positive on ALL 3 days individually (+1,200 / +3,600 / +2,850)
    b) Independent of other parameter changes
    c) Mechanistically sound (enables buy≤4/sell≥6 cycling vs dead-position accumulation)

  v31 had BUY_MAX=0 because SELL_MIN=7 never fired (max bid≈6).
  With SELL_MIN=6, bids of 6 are captured (~66% of ticks), enabling actual cycling.

All other v31 parameters unchanged.

Round 4 v31 — VEV passive market-making + cycling fix + VEV_5500 disable
=========================================================================
Base: v30 (524,038 on R4 30k local backtest; 53,689 on official R4 day 3).

Changes from v30
────────────────
1. Disable VEV_5500 buying  [clear PnL fix]
   VEV_5500 consistently generates -384: we buy 300 units at avg 4.4, but
   SELL_MIN=7 never fires (max observed bid ≈ 6). Position never unwinds.
...
class Trader:
    ...
    EXTREME_SELL: Dict[str, int] = {
        ...
        "VELVETFRUIT_EXTRACT":  5_272,   # v42: +8,089 (all days ✓)
        "VEV_4000":             1_262,   # v42: +3,006 (all days ✓)
        "VEV_4500":               764,   # v42: +2,418 (all days ✓)
        "VEV_5000":               267,   # v31: 272→267 (p60ish; cycles faster, TV-take handles ≥248)
        "VEV_5100":               180,   # v42c: 181→180 (+3,994; D2:-3047, more conservative)
        "VEV_5200":               105,   # v42: +2,941 (D2:-1131, others ✓)
        "VEV_5300":                58,   # v42b: 56→58 +1,626 additional (all days ✓)   # unchanged: 53→50 hurt -11.5k locally (premature sells)
        "VEV_5400":                19,
    ...
    EXTREME_BUY: Dict[str, int] = {
        "HYDROGEL_PACK":       10_020,   # v42b: 10023→10020 +6,638 additional (all days ✓)   # v29: official p20
        "VELVETFRUIT_EXTRACT":  5_232,   # v42: +2,061 (D1:-79, others ✓)
        "VEV_4000":             1_241,
        "VEV_4500":               738,   # v29b: reverted from 759
        "VEV_5000":               255,   # v42b: +4,265 (all days ✓)   # v29: official p20
        "VEV_5100":               163,   # v42: +2,009 (all days ✓)   # v29: official p20
        "VEV_5200":                91,   # v42: +5,380 (D3:-234, others ✓)   # v29: official p20
        "VEV_5300":                40,   # v42c: 39→40 (+2,358; D2:-981, D3:-241 — small)
        "VEV_5400":                13,   # v42: +3,931 (D3:-264, others ✓)
        # VEV_5500 removed: buying disabled (see Change 1)
    }

    EXTREME_CAPS: Dict[str, int] = {
        "HYDROGEL_PACK":       200,   # v29: raised from 180
        "VELVETFRUIT_EXTRACT": 200,   # v41: 180→200 (matches hard limit)
        "VEV_4000":            300,   # v41: →300 (extreme-only product, hard limit=300) (+14,811 on 30K, consistent all days)
        "VEV_4500":            300,   # v41: →300 (extreme-only product, hard limit=300) (+17,375 on 30K, consistent all days)
        "VEV_5000":            275,   # v41: →275 (TV-take also active, leave room) (+18,537 on 30K, consistent all days)
        "VEV_5100":            300,   # v41: →300 (extreme-only, +15,287 on 30K)
        "VEV_5200":            300,   # v41: →300 (extreme-only, +13,526 on 30K)
        "VEV_5300":            275,   # v41: →275 (TV-take + spread overlay active)
        "VEV_5400":            275,   # v41: →275 (TV-take + spread overlay active)
        "VEV_5500":            250,
    ...
    VEV_5500_BUY_MAX  = 4   # v39: re-enabled (sweep optimal: +7,650 across 30K backtest)
    VEV_5500_SELL_MIN = 6   # v39: 7→6 (sweep optimal: enables actual cycling)
