"""
Round 4 v37 — Extend timestamp cycle to VEV_5300 and VEV_5400
==============================================================
Base: v36 (495,923 local, 77,597 official).

Remaining official gap to Shane (82,597) ≈ 5,000:
  VEV_5300: us 3,136  vs Shane 7,124  → -3,988
  VEV_5400:                            → -1,245  (approx)
  VEV_5500:                            →   -472  (minor)

Changes from v36
────────────────
1. TS_CYCLE extended to include VEV_5300 and VEV_5400.
   Parameters from Shane's official log (official day 3 price path):
     VEV_5300: entry=55, low_ceiling=44, rebound=41, late=39
     VEV_5400: entry=19, low_ceiling=15, rebound=13, late=12
   Phase gating safety analysis:
   - VEV_5300 local Day 2: Phase 0 fires (max≈57 ≥ 55) but Phase 1 fails
     (mid never drops below 44 locally) → missed-window escape flattens.
   - VEV_5400 local Day 2: Phase 0 may fire (max≈19) but Phase 3 fails
     → escape handles safely.
   - Local Day 3 (official price level): all 4 phases fire as intended.

2. _trade_5300_5400_spread skipped when either 5300 or 5400 is cycle-controlled.
   Spread overlay adds conflicting orders when both legs target -300 in Phase 0.
   run() tracks which products were claimed by TS_CYCLE and suppresses spread.

v36 changes (retained)
────────────────────────
1. MARK01_IDS: {"Mark 01","Mark 14"} → {"Mark 01"} only
   Mark 14 is INFORMED — selling to him hurts us.

2. TS_CYCLE for VEV_5000/5100/5200/VELVETFRUIT (all retained from v36).
...
class Trader:
    ...
    # ── Timestamp cycle (official day 3 price shape) ─────────────────────────
    # Phases 0-3 are entry windows; phase 4 = flat/done; phase -1 = skipped day.
    # Format: list of (ts_window_end, mid_threshold, mid_direction, target_pos)
    # Phase transitions MUST be sequential — missing phase 0 skips entire cycle.
    # This prevents the cycle from misfiring on local days 1/2 where early
    # prices never reach the SHORT entry threshold.
    TS_CYCLE: Dict[str, list] = {
        ...
        # v37: added — closes remaining ~5,000 gap to Shane
        "VEV_5300": [
            (8_500,   55.0, "ge", -300),
            (45_500,  44.0, "le", +300),
            (74_500,  41.0, "ge", -300),
    ...
    # Window starts (must be past this ts to enter each phase)
    ...
    def run(self, state: TradingState):
        ...
        if velvet_fair is not None:
            cycle_claimed = set()   # v37: track products taken by TS_CYCLE
            for product in state.order_depths:
                if product.startswith("VEV_"):
                    if self._trade_ts_cycle(product, state, orders, td):
                        cycle_claimed.add(product)
                        continue   # timestamp cycle took over — skip other modules
            ...
            # v37: skip spread overlay when either leg is cycle-controlled —
            # the cycle targets ±300 independently; the spread adds conflicting
            # orders and is redundant during the timestamp cycle phases.
            if not (cycle_claimed & {"VEV_5300", "VEV_5400"}):
    ...
    def _trade_ts_cycle(
        ...
        through to normal v35 logic with no interference.
        ...
        if next_phase <= len(windows):
            ...
            if ts_start <= ts <= win_end:
                ...
                # We're stuck in current phase; next window already passed without
                # the price condition being met. Exit cleanly to flat rather than
                # holding a stranded position indefinitely.
                cyc["phase"] = 4
                cyc["target"] = 0
                phase  = 4
                target = 0
