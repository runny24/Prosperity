"""
Round 4 v27 — HYDROGEL sell threshold + VEV_5500 buy floor recalibrated to official data
==========================================================================================
Base: v26 (554,809 on R4 30k local backtest; 41,969 on official R4 day 3).

Changes from v26
────────────────
1. EXTREME_SELL[HYDROGEL_PACK]: 10,015 → 10,050

   Distribution mismatch from official R4 day 3 log:
     Local R4 data:   mean=9,994.7  → EXTREME_SELL=10,015 fires 21.9% ✓
     Official day 3:  mean=10,033.5 → EXTREME_SELL=10,015 fires 75.4% ✗
   At 75% fire rate the extreme module constantly sells HYDROGEL short at
   below-average prices. Threshold 10,050 ≈ official p90 → fires ~10%.
   EMA-based TV-take and passive quoting continue to generate PnL regardless.
   Local backtest impact: fires ~2.3% locally (acceptable trade-off).

2. VEV_5500_BUY_MAX: 5 → 4
   Official day 3: mean=4.5, p10=4, max=8.
   BUY_MAX=5 fired 73.9% of ticks — near-constant accumulation at/above mean.
   BUY_MAX=4 targets p10: fires ~10-15%, true extreme-low buys only.
   VEV_5500 was -434.5 on all prior versions; reducing accumulation is the fix.
   SELL_MIN stays at 7 (max bid=8 shows threshold is physically reachable).

Retained from v26:
   VEV_5100/5200 EXTREME_CAPS = 150  |  SELL_MIN=7  |  Mark01 overlay  |  TV unchanged

   Official R4 day 3 comparison:
     v17 (cap=200, BUY=165/93):  5100=+7,235  5200=+5,126  total=36,067
     v25 (cap= 30, BUY=153/85):  5100=+1,065  5200=+1,097  total=33,479
   v25 lost ~10k on 5100/5200 despite better thresholds — purely from the cap.

   Cap sweep on R4 30k backtester:
     cap= 30 → total 485,963
     cap=100 → total 526,868
     cap=150 → total 554,809  ← chosen
     cap=200 → total 581,761
   cap=150 captures 73% of the gain vs cap=200 with proportionally less
   end-of-day position exposure. VEV_5100 last mid = 141.5 (below BUY=153)
   and VEV_5200 last mid = 70.0 (below BUY=85) at end of day 3 — larger
   cap amplifies tail risk from open long positions.

   Threshold calibration unchanged (v25/R3_final values):
     v17 BUY thresholds (165/93) fire on 54% of ticks — near mean, not extreme.
     v25 BUY thresholds (153/85) fire on 27-35% of ticks, ~10pts below mean.
     Wider buy-sell spread (28pts vs 15pts for 5100) gives more PnL per cycle.

   Realized vs unrealized (cap=150, R4 30k backtest):
     VEV_5100: realized=+31,298  unrealized=+21,225  total=+52,523
     VEV_5200: realized=+22,484  unrealized=+10,500  total=+32,984

2. All v25 changes retained:
   - VEV_5500_SELL_MIN = 7 (sweep-confirmed: +1,291 vs R3_final baseline)
   - Mark 01 overlay (floors at EXTREME_SELL levels; additive size only)
   - TV values unchanged from R3_final (confirmed correct for R4 order book)
...
• VEV_5500 strict rule: buy ≤ BUY_MAX=5, sell ≥ SELL_MIN=7
...
class Trader:
    ...
    EXTREME_SELL: Dict[str, int] = {
        "HYDROGEL_PACK":       10_050,   # v27: raised from 10015 (official mean=10033; was firing 75%)
    ...
    EXTREME_CAPS: Dict[str, int] = {
        ...
        "VEV_4500":            200,
        "VEV_5000":            200,
        "VEV_5100":            150,   # v26: raised from 30
        "VEV_5200":            150,   # v26: raised from 30
        "VEV_5300":            250,
        "VEV_5400":            250,
        "VEV_5500":            250,
