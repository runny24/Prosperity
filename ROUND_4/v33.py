"""
Round 4 v33 — Bot-data-driven Mark 01 overlay + VEV_5500 re-enable
===================================================================
Base: v32 (473,657 local, ~56,822 projected official).

Deep analysis of all 4,281 local trades across 3 days revealed:

Mark 01 overlay was severely miscalibrated:
  VEV_5300: floor=53 fires only 3% of Mark 01's buys (13 units/3days).
             Mark 01's p75 bid = 43, p90 = 47, max = 53.
             floor=47 fires 13% → 63 units, avg sell 50.1, buy cost ~41 → +9.1/unit
  VEV_5400: floor=19 fires only 3% of Mark 01's buys (18 units/3days).
             Mark 01's p75 bid = 14, p90 = 17, max = 20.
             floor=15 fires 25% → 230 units, avg sell 16.5, buy cost ~13 → +3.5/unit
  Expected combined gain: +455 + 742 = +1,197 per 3 days

VEV_5500 re-enabled:
  Mark 22 sells at avg 3.89, Mark 01 bids ≥5 on 449 units (avg 6.2).
  Previous SELL_MIN=7 never fired (official max bid was 6). Lowered to 5.
  BUY_MAX=4 re-enabled. Expected: buy@~3.9, sell@~5.7 → +720 per 3 days.

Total projected improvement over v32: +1,917 per 3 days (≈ +640 on official day 3)
...
class Trader:
    ...
    VEV_5500_BUY_MAX  = 4   # v33: re-enabled; SELL_MIN lowered to 5 so sells now fire
    ...
    MARK01_MIN_BID: Dict[str, int] = {
        "VEV_5300":  47,   # v33: 53→47 (Mark 01's p90 bid; was firing only 3% of his buys)
        "VEV_5400":  15,   # v33: 19→15 (Mark 01's p75 bid; was firing only 3% of his buys)
