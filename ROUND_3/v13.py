"""
Round 3 v13 — VELVETFRUIT size increase (friend's v10 improvement)
====================================================================
CHANGES FROM v12:

v12 baseline: ~10,100 expected (HYDROGEL EMA fix from v11's -1,193)
v13 target:   ~10,490 (matching friend's 432879 result)

SINGLE CHANGE:
  VELVETFRUIT_EXTRACT MAX_TAKE_SIZE: 30 → 50  (+~340 PnL)
  VELVETFRUIT_EXTRACT MAX_MAKE_SIZE: 36 → 50  (same gain, larger passive quotes)

WHY THIS WORKS (confirmed from friend's v10 log 432879.log):
  - VELVETFRUIT NPC spread is almost always 5 ticks (72% of ticks)
  - With spread=5, our passive quotes land at best_bid+1 / best_ask-1
  - Larger size per quote → more PnL per fill, same fill rate
  - Friend v9 (size=30/36): +2,754 VEV PnL
  - Friend v10 (size=50/50): +3,092 VEV PnL  → delta +338
  - TAKE_EDGE=2 kept (take_edge=1 caused inventory build-up per friend's plan)

PROGRESSION:
  v11 (432353): +8,311  ← wrong HYDROGEL anchor, wrong sizes
  v12 (pending): ~10,100  ← HYDROGEL EMA fix
  v13 (this):  ~10,490  ← VELVETFRUIT size bump to match friend's v10

All other v12 parameters unchanged:
  - HYDROGEL: EMA alpha=0.08, MAKE_EDGE=10, MAX_MAKE_SIZE=24, TAKE_EDGE=8
  - VEV time_value model: {5300:50, 5400:16, 5200:35, 5100:8, 5000:5}
  - VEV_5300/5400 spread overlay (SPREAD_HIGH_SELL=36.5)
  - VEV_5500 strict rule (buy ≤5, sell ≥8)
  - Correct limits: HYDROGEL=200, options=300
"""
...
class Trader:
    ...
    MAX_MAKE_SIZE: Dict[str, int] = {
        ...
        "VELVETFRUIT_EXTRACT": 50,   # ~25% of limit per tick (friend's v10 calibration)
    }
