"""
Round 3 v14 — Feature-flag testbed on top of v13 baseline (~10,490)
====================================================================
Toggle the three flags below to test each improvement independently.
All False = identical behaviour to v13 (friend's 432879 result).

FEATURE_DYNAMIC_TIME_VALUE
  Each voucher's fair = intrinsic + time_value * sqrt(T / T0)
  T0 = 10000/30000 (day-2 start). Time value shrinks toward expiry.
  Effect: buys more aggressively early in the day when BS fair > 50;
  stops buying near expiry when the option is nearly worthless.
  Day tracking via prev_ts rollover (same method as v10).

FEATURE_LOWER_5300_SELL_EDGE
  VEV_5300 sell take-edge: 3 → 2  (sell when bid > 52 instead of > 53).
  All other products' sell edges unchanged.
  Effect: more sell fills → faster cycling → more realized PnL per day.

FEATURE_SCALE_VEV_5200
  VEV_5200 SOFT_LIMIT: 50 → 100.
  v13 baseline earns +86 with 50 short contracts.
  Effect: ~doubles short exposure → expected ~+172 additional PnL.
"""
...
# ── EXPERIMENT FLAGS ──────────────────────────────────────────────────────────
# Baseline (all False) ≈ +10,490  (matches friend's 432879 platform result)
# Toggle one at a time to isolate each improvement's contribution.

FEATURE_DYNAMIC_TIME_VALUE   = False   # time_value * sqrt(T/T0) — day-aware decay
FEATURE_LOWER_5300_SELL_EDGE = False   # VEV_5300 sell edge 3 → 2
FEATURE_SCALE_VEV_5200       = True   # VEV_5200 SOFT_LIMIT 50 → 100
# ─────────────────────────────────────────────────────────────────────────────
...
class Trader:
    ...
    # FEATURE_SCALE_VEV_5200 doubles VEV_5200 from 50 → 100
    SOFT_LIMITS: Dict[str, int] = {
        "VEV_5000": 80,
        "VEV_5100": 30,
        "VEV_5200": 100 if FEATURE_SCALE_VEV_5200 else 50,
        "VEV_5300": 300,
        "VEV_5400": 300,
        "VEV_5500": 300,
    }
    ...
    # With FEATURE_DYNAMIC_TIME_VALUE these are multiplied by sqrt(T/T0).
    ...
    # FEATURE_LOWER_5300_SELL_EDGE: VEV_5300 sell edge 3 → 2
    TAKE_EDGE_SELL: Dict[str, float] = {
        ...
        "VEV_5100": 3.0,
        "VEV_5200": 3.0,
        "VEV_5300": 2.0 if FEATURE_LOWER_5300_SELL_EDGE else 3.0,
        "VEV_5400": 1.0,
        "VEV_5500": 0.5,
    }
    ...
    # ── FEATURE_DYNAMIC_TIME_VALUE: time constants ─────────────────────────────
    TOTAL_TICKS   = 30000
    TICKS_PER_DAY = 10000
    T0            = 10000 / 30000   # fraction of sim remaining at day-2 start
    ...
    def run(self, state: TradingState):
        ...
        # ── Day counter + T (only needed for dynamic time value) ───────────────
        if FEATURE_DYNAMIC_TIME_VALUE:
            day     = td.get("day", 0)
    ...
    # ── Voucher: intrinsic + (optionally time-decayed) time value ─────────────
    def _trade_voucher(
        self, product: str, underlying_fair: float,
        state: TradingState, T: Optional[float] = None
    ) -> List[Order]:
        ...
        base_tv    = self.VOUCHER_TIME_VALUE[product]

        if FEATURE_DYNAMIC_TIME_VALUE and T is not None:
            # Scale time value by sqrt(T / T0): decays toward expiry
            tv_scale   = math.sqrt(max(0.0, T / self.T0))
            time_value = base_tv * tv_scale
        else:
            time_value = base_tv
