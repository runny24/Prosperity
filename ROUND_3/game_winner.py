"""
Round 3 v17 — Robust combined strategy
=======================================
Base:   445307 (~13,459) with baked-in LOWER_5300_SELL_EDGE (+104 confirmed)
Plus:   Strategy C (+3,075): HYDROGEL extreme thresholds fixed
Plus:   Strategy D (+1,387): VEV_5000 sell edge 6→3 + extreme buy
Plus:   50k robust extracts: VELVETFRUIT extreme, VEV_4000/4500 extreme,
        VEV_5300/5400 extreme recalibration, tighter spread overlay,
        more aggressive MAKE_EDGE, TV-take / extreme conflict fix.

Key design decisions
────────────────────
• VEV_5100 / VEV_5200: disabled for TV-take (DISABLED_VOUCHERS). Extreme
  module kept but capped at 30 units — thresholds are only ±5–10 pts from
  fair, so a ±20pt VEV move would cause 97-100% trigger rate → catastrophic
  if uncapped.  Cap of 30 limits worst-case loss to ~300 per product.

• VEV_5000 asymmetric take edges: BUY=6 (ask < 261), SELL=3 (bid > 270).
  This 3× increase in sell opportunities enables proper inventory cycling.

• Conflict resolution: any product in DISABLED_VOUCHERS is NOT sent through
  _trade_voucher (TV-take off).  Extreme module runs independently for all
  products in EXTREME_CAPS.

• No timestamp-based logic anywhere. All thresholds are price-level only.
  Trigger rates validated at 7–17% of ticks (not path-locked).

• No feature flags — clean single-strategy submission.

Expected PnL breakdown (conservative):
  Base 445307 + 5300_sell_edge:  ~13,600
  Strategy C (HYDROGEL extreme):  +3,075
  Strategy D (VEV_5000 tuning):   +1,387
  VELVETFRUIT extreme (50k):      +2,000   (was ~0 in v16)
  VEV_4000/4500 extreme (50k):    +1,500   (delta≈1, robust cross-day)
  VEV_5300/5400 recalibrated:     +1,000
  Spread overlay tightened:         +300
  MAKE_EDGE improvement:            +500
  VEV_5100/5200 (capped at 30):   +200-500 (limited upside, limited risk)
  ─────────────────────────────────────────
  Estimated total:               ~23,500–24,000
"""
...
class Trader:
    ...
    EXTREME_SELL: Dict[str, int] = {
        ...
        "VEV_4000":             1_269,   # 50k calibration: ~0.2% (fires infrequently)
        "VEV_4500":               769,   # 50k calibration: ~0.2%
    ...
    EXTREME_BUY: Dict[str, int] = {
        ...
        "VEV_4000":             1_255,   # 50k calibration: ~2.1%
        "VEV_4500":               755,   # 50k calibration: ~3.2%
