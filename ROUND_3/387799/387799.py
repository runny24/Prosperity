"""
Round 3 v2 — Fixed Limits + Dynamic Fair
==========================================
Fixes from v1:
...
Strategy:
  ...
  Options: penny-inside around market mid, BS for reference
"""
...
class Trader:
    ...
    def _trade_option(self, state, td, product, vev_mid, T):
        ...
        # Compute BS fair from underlying
        if vev_mid is not None and vev_mid > 0:
            bs_fair = bs_call(vev_mid, strike, T, self.VOL)
        else:
            bs_fair = market_mid

        # Use BS fair for options (it tracks the underlying correctly)
        fair = bs_fair
