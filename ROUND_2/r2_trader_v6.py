"""
IMC Prosperity Round 2 — Trader v6
====================================
...
v6 (this file) — v3 base + aggressive conversion handling
  - IPR: identical to v3
  - ACO: identical to v3
  - Conversions: CONV_MIN_EDGE lowered 2.0 → 0.5, CONV_MAX_UNITS raised 10 → 40
    Rationale: all tested logs have empty conversionObservations ([{}, {}]),
    so conversions always returned 0 in testing. The live competition environment
    may populate observations — if so, we want to exploit every tick of edge
    available (access fee buys us +25% volume so any positive edge is worth taking).
    Logging added so we can diagnose whether observations are populated in the live run.
  - Conversion direction note:
      buy conversion:  net cost = askPrice + transportFees + importTariff
      sell conversion: net revenue = bidPrice - transportFees - exportTariff
    We convert-buy when (FAIR - buy_cost) > CONV_MIN_EDGE
    We convert-sell when (sell_revenue - FAIR) > CONV_MIN_EDGE
"""
