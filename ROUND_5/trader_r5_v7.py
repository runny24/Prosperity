"""
Round 5 v7 — Optimized Parameters for v5 Whitelist
===================================================
Based on v5 with refined parameters for better risk-adjusted returns.

Key changes from v5:
  1. Signal thresholds: Slightly tighter bands for more frequent entries
  2. Circuit breaker: More responsive to drawdowns
  3. Position ramping: Slightly faster to capture moves
...
class Trader:
    ...
    # ── Whitelist: ONLY these 6 products are traded (proven in v5) ───────
    WHITELIST = {
    ...
    # ── OPTIMIZED: Tighter signal thresholds for more entries ───────────
    SIGNAL_STRONG = 50   # → ±LIMIT (10 units) [was 55]
    SIGNAL_WEAK   = 18   # → ±HALF_POS (5 units) [was 20]
    WARMUP_TICKS  = 2500 # timestamps before using signal [was 3000]

    # ── OPTIMIZED: Circuit breaker ────────────────────────────────────────
    DRAWDOWN_PTS   = 550   # pts drawdown before pausing [was 600]
    COOLDOWN_TICKS = 1800  # timestamps locked out [was 2000]

    # ── Order sizing ─────────────────────────────────────────────────────
    PASSIVE_SIZE  = 2
    RAMP_PER_TICK = 6   # [was 5]
