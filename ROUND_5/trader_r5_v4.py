"""
Round 5 v4 — Whitelist-only EMA Trend Following
=================================================
BACKTEST RESULTS (v3, Days 2-4):
  - Total: -1,062,711 across 46 products
  - 40 products were net losers, destroying value with false EMA signals

ROOT CAUSE:
  The dual-EMA crossover works well when a product trends cleanly, but most
  products mean-revert or trend against us on the days tested. Paying the
  spread to aggressively enter/exit positions on 40 losing products wipes out
  the gains from the 6 winning ones.

WHITELIST ANALYSIS (backtest, all 3 days consistently positive):
  PEBBLES_XL              +171,821  (all 3 days: +9563 / +74084 / +88174)
  OXYGEN_SHAKE_MORNING_BREATH +37,722  (all 3 days: +7305 / +22226 / +8191)
  ROBOT_MOPPING            +20,172  (all 3 days: +1858 / +9502 / +8812)
  GALAXY_SOUNDS_BLACK_HOLES +19,797  (all 3 days: +7094 / +8504 / +4199)
  SLEEP_POD_COTTON         +17,402  (all 3 days: +5561 / +3529 / +8312)
  MICROCHIP_CIRCLE         +12,639  (all 3 days: +5515 / +3668 / +3456)
  ─────────────────────────────────
  WHITELIST TOTAL         +279,553

  Everything else:       -1,342,264  ← eliminated

V4 CHANGE (single line of logic):
  Only generate orders for products in WHITELIST.
  All 44 other products → empty order list, no position ever taken.
"""
...
class Trader:
    ...
    # ── Whitelist: ONLY these products are traded ────────────────────────
    # Selected by backtesting Days 2-4: each product profitable on all 3 days.
    WHITELIST = {
        "PEBBLES_XL",                    # +171,821 (constraint-based)
        "OXYGEN_SHAKE_MORNING_BREATH",   # +37,722
        "ROBOT_MOPPING",                 # +20,172
        "GALAXY_SOUNDS_BLACK_HOLES",     # +19,797
        "SLEEP_POD_COTTON",              # +17,402
        "MICROCHIP_CIRCLE",              # +12,639
    }

    # ── Dual-EMA parameters ──────────────────────────────────────────────
    FAST_ALPHA = 0.03    # half-life ≈ 23 steps ≈ 2300 timestamps
    SLOW_ALPHA = 0.003   # half-life ≈ 231 steps ≈ 23100 timestamps

    SIGNAL_STRONG = 60   # → ±LIMIT (10 units)
    SIGNAL_WEAK   = 25   # → ±HALF_POS (5 units)
