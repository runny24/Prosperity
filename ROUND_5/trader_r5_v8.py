"""
Round 5 v8 — Conservative Refinement of v5
=============================================
Based on v5 with minimal, risk-focused parameter adjustments.

Key changes from v5:
  1. Slightly wider signal bands to reduce overtrading
  2. More conservative circuit breaker
  3. Same whitelist (proven to work)
...
class Trader:
    ...
    # ── CONSERVATIVE: Slightly wider bands to reduce overtrading ─────────
    SIGNAL_STRONG = 58   # → ±LIMIT (10 units) [was 55]
    SIGNAL_WEAK   = 22   # → ±HALF_POS (5 units) [was 20]
    WARMUP_TICKS  = 3500 # timestamps before using signal [was 3000]

    # ── CONSERVATIVE: Circuit breaker ─────────────────────────────────────
    DRAWDOWN_PTS   = 700   # pts drawdown before pausing [was 600]
    COOLDOWN_TICKS = 2500  # timestamps locked out [was 2000]
