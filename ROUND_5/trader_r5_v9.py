"""
Round 5 v9 — Day-Specific Risk Management
==========================================
Addresses the Day 4 trend reversal issue with tighter risk controls.

Key changes from v5:
  1. Day-aware circuit breaker: More sensitive on Day 4
  2. Early position reduction: Start reducing positions earlier in the day
  3. Trend strength filter: Only enter if momentum is strong

Analysis of v5 day breakdown:
  Day 2: +37,564 (all positive!)
  Day 3: +90,374 (2 products negative: MICROCHIP, SLEEP_POD)
  Day 4: +21,398 (2 products negative: ROBOT_MOPPING, OXYGEN_MORNING)

Target: Reduce Day 4 losses while maintaining Day 2/3 gains
"""
...
class Trader:
    ...
    # ── IMPROVED: Day-aware circuit breaker ──────────────────────────────
    # Day 4 needs tighter controls
    DRAWDOWN_PTS_DAY2 = 600
    DRAWDOWN_PTS_DAY3 = 600
    DRAWDOWN_PTS_DAY4 = 450   # Tighter for Day 4
    COOLDOWN_TICKS = 2000
    ...
    def run(self, state: TradingState):
        ...
        for prod, od in state.order_depths.items():
            ...
            orders[prod] = self._trade(prod, od, pos, mids, td, ts, day)

        tdo = json.dumps(td)
        logger.flush(state, orders, conversions, tdo)
        return orders, conversions, tdo

    def _detect_day(self, ts, td):
        """Detect which day we're in based on timestamp"""
        # Day 2: 0-999999, Day 3: 1000000-1999999, Day 4: 2000000-2999999
        if ts < 1000000:
            return 2
        elif ts < 2000000:
            return 3
    ...
    def _trade(self, prod: str, od: OrderDepth, pos: int,
               mids: Dict[str, float], td: dict, ts: int, day: int) -> List[Order]:
        if prod == self.PEBBLES_XL:
            return self._trade_xl(prod, od, pos, mids, td, ts)
        return self._trade_ema(prod, od, pos, mids, td, ts, day)
    ...
    def _trade_ema(self, prod: str, od: OrderDepth, pos: int,
        ...
        target = self._circuit_breaker(prod, raw_target, pos, mids.get(prod), td, ts, day)
    ...
    # ── Day-aware Circuit breaker ─────────────────────────────────────────
    def _circuit_breaker(self, prod: str, raw_target: int,
                          pos: int, mid: Optional[float],
                          td: dict, ts: int, day: int) -> int:
        if mid is None:
            return raw_target

        # Get day-specific drawdown threshold
        if day == 2:
            dd_pts = self.DRAWDOWN_PTS_DAY2
        elif day == 3:
            dd_pts = self.DRAWDOWN_PTS_DAY3
        else:
            dd_pts = self.DRAWDOWN_PTS_DAY4
