...
class Trader:
    """
    Round 3 v8 milestone candidate.

    Goal:
    - Beat r3_v3 platform result (+7731) and cross 10000 if possible.

    Built from r3_v3, our best platform version:
    - Disable weak vouchers: VEV_4000, VEV_4500, VEV_5500, VEV_6000, VEV_6500.
    - Use tiny probe positions in VEV_5100 / VEV_5200 only.
    - Disable VEV_5500 after unstable/negative platform contribution.
    - Keep VEV_5300 and VEV_5400 core trading from v3.
    - Add small relative-value overlay on VEV_5300 - VEV_5400 spread.
    - Add zero-bid lottery orders for VEV_6000 / VEV_6500.
    ...
    SPREAD_LOW_BUY = 24.0
    ZERO_BID_LOTTERY = {"VEV_6000", "VEV_6500"}
    ...
    def run(self, state: TradingState):
        ...
        if velvet_fair is not None:
            ...
            self._trade_zero_bid_lottery(state, result)
    ...
    def _trade_zero_bid_lottery(self, state: TradingState, result: Dict[str, List[Order]]) -> None:
        for product in self.ZERO_BID_LOTTERY:
            if product not in state.order_depths:
                continue
            orders = result.setdefault(product, [])
            buy_capacity = self._remaining_capacity(product, state, orders)[0]
            if buy_capacity <= 0:
                continue
            # Historical trades for these far OTM vouchers frequently print at 0.
            # A zero bid is non-crossing against the normal ask=1 book, so it should
            # only fill if bots are willing to sell for free.
            orders.append(Order(product, 0, buy_capacity))
