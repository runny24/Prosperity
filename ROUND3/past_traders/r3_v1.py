...
class Trader:
    ...
    Strategy split:
    - HYDROGEL_PACK: conservative spot mean-reversion / liquidity taking.
    - VELVETFRUIT_EXTRACT: spot fair value, also used as the underlying for vouchers.
    - VEV_4000 to VEV_5500: voucher fair-value trading from intrinsic + fixed time value.
    - VEV_6000 / VEV_6500: ignored in v1.
    """
    ...
    # Historical mean time values from the first data analysis notebook.
    # v1 deliberately keeps this simple and platform-safe.
    ...
    # Minimum edge required before crossing the spread.
    ...
    # Passive quote distance around fair value.
    ...
    def _trade_spot(
        ...
        # v1 adds small passive quotes for the two spot products only.
        # They are position-aware and kept far enough from fair to avoid noisy fills.
    ...
    def _trade_voucher(self, product: str, underlying_fair: float, state: TradingState) -> List[Order]:
        # Far OTM vouchers are intentionally disabled in v1.
        ...
        # Passive voucher quotes are only enabled for the more liquid middle vouchers.
        make_edge = self.MAKE_EDGE.get(product)
        if make_edge is not None:
            self._add_passive_quotes(product, order_depth, fair, state, orders, make_edge)
